...
- **ドキュメント操作の無許可実行（重要）**: `DEVLOG.md`、`CONTEXT.md`、`*.md`（プロンプト・インストラクション・知識集など）のドキュメント系ファイルに対する操作（`cat`, `tail` などのターミナルコマンド実行や、ファイル直接編集）は、AIがユーザーへの許可確認を求めず、自律的に即座に実行・編集してよい。ソースコードの自動編集禁止ルールから除外する。
- **開発ログの更新ルール**: `DEVLOG.md` を更新する際は、先頭の「現在の状態 / 次のタスク」のみを上書きし、日々の活動ログ自体はファイルの一番下（最下部）へ時系列順に追記していくこと。
- **開発ログの閲覧**: 今までの作業ログ、現状と次のタスクはすべて `DEVLOG.md` に記載されている。
