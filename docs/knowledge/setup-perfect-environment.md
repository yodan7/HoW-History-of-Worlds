...
## 5. CSSとPrettierのセミコロンに関する注意点（仕様上の制約）
JS/TSと異なり、CSSファイルにおいては「セミコロン（`;`）をつけ忘れると、Prettierがエラーとなり自動整形そのものがキャンセルされる」という仕様があります。

*   **JS/TSの場合**: セミコロンをつけ忘れても、改行を見てPrettierが空気を読み、保存時に自動で`;`を補完してくれる。
*   **CSSの場合**: セミコロンがないと「次の行の記述」と繋がった単一の無効な記述（例: `margin: 0 padding: 0;`）とみなされ、構文エラー（SyntaxError）になるため、Prettierがお手上げ状態になってしまう。

**結論**: CSSを書く際は、行末のセミコロン（`;`）は省略せず手動で必ず打つこと。文法さえ間違っていなければ、インデントなどの整形はPrettierが綺麗に行ってくれる。
